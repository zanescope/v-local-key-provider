...
func (collector *Collector) scanInternalXORKeys(data []byte) {
	for _, key := range v4InternalXORKeys(data) {
		...
		collector.internalXORKeys = append(collector.internalXORKeys, collector.runtime.CloneSensitive(key))
		clearBytes(key)
...
func findV4DatabasePassphrase(candidates [][]byte, page databasePage, remaining budget) ([]byte, int) {
	return findV4DatabasePassphraseWithRuntime(candidates, page, remaining, DefaultRuntime().normalized())
}

func findV4DatabasePassphraseWithRuntime(candidates [][]byte, page databasePage, remaining budget, acquisitionRuntime Runtime) ([]byte, int) {
	if len(candidates) == 0 {
		return nil, 0
	}
	...
	for worker := 0; worker < workerCount; worker++ {
		go func() {
			...
			for candidate := range jobs {
				...
				tested.Add(1)
				_, valid := acquisitionRuntime.verifyDatabasePassphrase(candidate, page.Data, &found, remaining)
				if valid &&
					found.CompareAndSwap(false, true) {
					result <- acquisitionRuntime.CloneSensitive(candidate)
					return
				}
			}
		}()
...
func (collector *Collector) xorPassphraseCandidates(candidates [][]byte) [][]byte {
	...
	for _, candidate := range candidates {
		for _, xorKey := range collector.internalXORKeys {
			...
			if collector.runtime.MarkSensitive != nil {
				collector.runtime.MarkSensitive(passphrase)
			}
			...
			if seen[fingerprint] {
				collector.runtime.ClearSensitive(passphrase)
				continue
			}
...
func (collector *Collector) resolveDatabasePassphrase(remaining budget) {
	...
	preferredTransformed := collector.xorPassphraseCandidates(collector.binaryCandidates)
	fallbackTransformed := collector.xorPassphraseCandidates(collector.binaryFallbackCandidates)
	defer func() {
		for _, group := range [][][]byte{preferredTransformed, fallbackTransformed} {
			for _, candidate := range group {
				collector.runtime.ClearSensitive(candidate)
			}
		}
	}()
	groups := [][][]byte{
		preferredTransformed,
		fallbackTransformed,
	...
	for _, group := range groups {
		...
		var groupTested int
		candidate, groupTested = findV4DatabasePassphraseWithRuntime(group, page, remaining, collector.runtime)
		tested += groupTested
		if len(candidate) == 32 {
			break
		}
	...
	defer collector.runtime.ClearSensitive(candidate)
...
func (collector *Collector) recordGlobalPassphraseWithin(candidate []byte, source string, completeCallEvidence bool, remaining budget) bool {
	if len(candidate) != 32 {
		return false
	}
	...
	if evidence == nil {
		evidence = &globalPassphraseEvidence{
			secret: collector.runtime.CloneSensitive(candidate), paths: map[string]bool{}, sources: map[string]bool{},
		}
	...
	defer func() {
		for _, value := range derived {
			collector.runtime.ClearSensitive(value.key)
		}
	}()
	for _, target := range collector.targets.Pages {
		...
		for _, profile := range collector.runtime.Profiles {
			...
			if !cached.computed {
				...
				cached.key = providercrypto.DeriveProfileKey(
					profile, candidate, target.Data[:profile.PlaintextHeaderSize],
					collector.runtime.cryptoRuntime(nil, remaining),
				)
			}
			...
			if !providercrypto.VerifyRawKeyWithProfile(
				profile, cached.key, target.Data, collector.runtime.cryptoRuntime(nil, remaining),
			) {
				continue
			}
...
func (collector *Collector) collectKeyObjects(data []byte, seen map[uint64]bool, readAt func(pointer uint64, buffer []byte) int) {
...
func (collector *Collector) hasCandidatesForSalt(salt string) bool {
	found := false
	for _, target := range collector.targets.Pages {
...
func (collector *Collector) scanDatabasePatterns(data []byte) {
...
func (collector *Collector) scan(data []byte) {
