...
func DecodeRequestData(data []byte) (AcquireRequest, error) {
	...
	case "prepare", "observe", "finalize", "cancel", "revalidate_security_posture":
	default:
		return AcquireRequest{}, errors.New("workflow.operation 无效")
	}
	switch request.Workflow.Operation {
	case "prepare":
		if request.Workflow.SessionID != "" || request.Workflow.ExpectedCatalogID != "" || request.Workflow.ActionReceipt != nil {
	...
	case "cancel":
		if request.Workflow.SessionID == "" || request.Workflow.ActionReceipt != nil {
