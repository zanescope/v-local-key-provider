...
> 本文只记录旧方案到当前规范的迁移 provenance，不定义运行时契约或能力状态。
...
| 本地授权数据、Provider 不联网、不修改微信、凭据不进日志 | 加强 | 3、14、18、19.5；增加 IPC、helper 信任、crash/core dump 和路径越界防护。 |
| media 阶段 | 加强 | 16.5、17.3、19.4；与 database coverage 分离，但在请求 media scope 时纳入总体结果。 |
| 不引入 Skill | 保留 | 3.2；Agent 只消费 v1 机器协议，不依赖对话记忆或外部 Skill 保存凭据。 |
...
| 最后依据真实耗时调预算和候选上限 | 保留 | 在真机证据完成后校准。 |
