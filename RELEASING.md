...
2. 手动触发真机回归工作流（`live-regression.yml`），明确提供候选 run id 和源码提交。该文件尚未进入默认分支、无法直接 dispatch 时，可通过默认分支已注册的 `Audit gates` 传入不含私有路径的 `live_regression_request`，调用同一 reusable workflow；普通 audit jobs 在该模式下不会运行。真机工作流会跨 workflow 下载那一份不可变候选，复核清单中的全部摘要，并以 `gh attestation verify --signer-workflow .../release-candidate.yml` 验证来源；禁止在真机 runner 重新构建替代品。账号目录和数据库目录不得进入 GitHub input、Secret 或环境变量：Windows runner 只从 `%LOCALAPPDATA%\v-local\live-regression-private\config.json` 读取，macOS runner 只从 `~/Library/Application Support/v-local/live-regression-private/config.json` 读取。该文件必须是 schema v1、仅含 `schema_version`、`account_dir`、`db_dir`，并满足平台专用的 owner、ACL/mode 和非 reparse/symlink 校验。真机固定同时请求 database 与 media；成功输出必须是脱敏 schema v1 正式证据，并记录候选来源、目标微信身份、实际进程架构、registry route、两类完整覆盖和验证过的 cipher profile。失败时只允许上传另一个 schema v1 的 qualification-only 诊断；该诊断不得包含目标摘要、版本、路径、账号身份、原始 Provider 响应、密钥候选或密钥，也不得被 promotion 引用。
